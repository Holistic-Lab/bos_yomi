...
Transformer la carte en **décision de lancement** : pour chacun des 4 verticaux, savoir (a) combien de prospects réels à Tours/agglo, (b) l'économie réelle du produit d'ancrage (coût de livraison vs prix vendable → marge et MRR par client), (c) combien de clients pour atteindre 3-5 k€/mois **ET combien de clients Karim peut servir seul** (temps de livraison + maintenance mensuelle par client → plafond de capacité solo ; si le plafond arrive avant l'objectif, le modèle est invalide tel quel), (d) le risque de churn, (e) la concurrence locale déjà active. But final : **par quel vertical + quel produit d'ancrage Karim pousse-t-il sa première porte, et quel est le chemin chiffré vers 3-5 k€/mois récurrents.**
...
Lance en parallèle :
- **Agent 1 — Densité locale Tours/agglo** (sonnet) : compter les prospects réels par vertical dans le 37 (annuaires Pages Jaunes/Google Maps/SIRENE ; **NB : BOS a déjà des exports SIRENE pour plombiers 43.22A/B, électriciens 43.21A, garages 45.20A — les réutiliser, se concentrer sur paysagistes 81.30Z, boulangeries 10.71C/47.24Z, véto 75.00Z, toiletteurs 96.09Z**). Sortie : nombre d'établissements par vertical et par zone (Tours intra / agglo), **en distinguant indépendants vs chaînes/franchises** (une Marie Blachère ou un centre auto de réseau n'est PAS une cible — le marché réel = les indépendants).
- **Agent 2 — Économie de l'ancrage BTP + garages** (opus) : coût réel de livraison (n8n/Make, téléphonie Twilio, LLM, hébergement, SMS) pour chaque ancrage BTP + garage ; prix vendable observé sur le marché FR (SaaS ET prestataires locaux comparables : agences web en retainer, infogéreurs) ; marge nette par client ; MRR atteignable ; nombre de clients pour 3-5 k€/mois. **Inclure l'estimation du temps humain : heures de setup par client + heures de maintenance/mois par client (pour un solo outillé Claude Code) → plafond de clients gérable seul.** Chiffré, sourcé, estimations marquées comme telles.
- **Agent 3 — Boulangerie : validation douleur + économie** (opus) : chercher des preuves NON-vendeurs du volume/valeur des commandes spéciales en boulangerie-pâtisserie-traiteur FR (forums pros type La Toque / groupes Facebook de boulangers / Confédération CNBPF / Observatoire Fiducial / presse métier) ; l'agent commandes spéciales est-il un vrai besoin ou une invention d'agence ? + économie de l'ancrage (mêmes exigences que l'Agent 2, temps humain inclus). Verdict GO/NO-GO sur le pari boulangerie.
- **Agent 4 — Churn & durée de vie des abonnements** (opus) : LE plus gros angle mort. **Source d'analogie prioritaire : les MSP (managed service providers / infogéreurs IT pour PME) — exactement le même modèle (prestataire technique local, abonnement mensuel, TPE clientes) avec 20 ans de données publiques (churn annuel, durée de vie client, pricing, causes de résiliation, ce qui fidélise).** En complément : rétention des SaaS TPE FR et des abonnements d'automatisation quand des données existent. Traduire en règles de design de l'offre pour minimiser le churn (ce qui fait tenir : volume vécu, dépendance opérationnelle, relation locale, itération continue).
- **Agent 5 — Concurrence locale Tours + outsider véto/toiletteur** (sonnet) : existe-t-il des agences IA / prestataires déjà actifs sur ces verticaux à Tours/Centre-Val de Loire ? + traiter l'outsider véto/toiletteur (réglementation données santé animale d'abord, puis douleur/ancrage/récurrence, **+ économie sommaire de l'ancrage — version allégée du cadre Agent 2, pour que la fiche véto/toiletteur soit comparable aux autres**). Verdict : véto viable ou repli sur toiletteur seul.
...
1. **Une fiche par vertical** : densité locale (nb de prospects indépendants Tours/agglo) · produit d'ancrage retenu · économie (coût livraison, prix vendable, marge, MRR/client) · temps humain (setup + maintenance/mois par client) · nb de clients pour 3-5 k€/mois **vs plafond de clients gérable en solo** · risque de churn + parade · concurrence locale · verdict GO / GO prudent / NO-GO.
2. **L'arbitrage des ancrages BTP** : noter chacun des 3 candidats (vocal / relance devis+avis / relance saisonnière) sur la grille : répond à une douleur forte · valeur perçue · facile à livrer · récurrent · défendable anti-fait-soi-même · **démoable en ~10 min devant le gérant avec SES propres données** (le critère qui rend l'audit vendeur). Un gagnant, justifié.
3. **Les règles de design de l'offre anti-churn** (issues de l'Agent 4, benchmarks MSP en tête) — comment structurer l'abonnement pour qu'il tienne.
4. **La reco de LANCEMENT** : par quel vertical + quel produit d'ancrage Karim démarre le terrain, le chemin chiffré vers 3-5 k€/mois, et l'ordre d'attaque des autres verticaux. **Cette reco doit explicitement réconcilier les chantiers existants** : le kit de démarchage vocal déjà prêt (listes SIRENE, calculateur ROI, playbook — nuit du 19/07) et la deadline dure AutoClim (démo 18/08) s'intègrent dans la stratégie audit-first — le vocal est UN ancrage possible parmi d'autres, pas un chantier parallèle. Dire ce qu'on garde, ce qu'on adapte, ce qu'on met en pause.
5. **Verdict outsider véto/toiletteur** : dans la course ou écarté, et pourquoi.
6. Les risques résiduels et ce qui ne pourra être tranché que par le terrain (les 20 premiers audits).
